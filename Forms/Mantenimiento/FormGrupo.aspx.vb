    ...
    Protected Sub btnOK_Click(sender As Object, e As EventArgs)
        ...
        Try
            If (valor.Equals("0") Or valor.Equals("")) Then
                ...
                If BD.ExisteGrupo(tbGrupo.Text.Trim, CInt(Session("IdUsuario"))) = 1 Then
                    message = "El grupo que desea ingresar ya existe"
                    Funciones.EjecutarFuncionJavascript(Me, "window.parent.showNotification2('" + message + "','A');")
                    Return
                End If

                BD.spGrupoIngresar(tbGrupo.Text.ToUpper, tbDescripcion.Text.ToUpper, idUser, nmUser)
            ...
            Else
                If tbGrupo.Text = "" Then
                    message = "Debe ingresar un nombre de grupo para continuar"
                    Funciones.EjecutarFuncionJavascript(Me, "window.parent.showNotification2('" + message + "','A');")
                    Return
                End If

                If tbDescripcion.Text = "" Then
                    message = "Debe ingresar una descripcion del grupo para continuar"
                    Funciones.EjecutarFuncionJavascript(Me, "window.parent.showNotification2('" + message + "','A');")
                    Return
                End If

                BD.spGrupoActualizar(CInt(tbID.Text), tbGrupo.Text.ToUpper, tbDescripcion.Text.ToUpper, idUser, nmUser)
