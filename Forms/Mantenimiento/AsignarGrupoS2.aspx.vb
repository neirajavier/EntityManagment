...
Public Class AsignarGrupoS2
            ...
            Try
                For Each row As DataRow In tableG.Rows
                    If (it.Index = valor And it.Selected = True) Then
                        GruposAsig = BD.spGrupoSubUsuariosConsultar2(CInt(Session("IdUsuario")), CInt(row("sid").ToString), CInt(it.Value.ToString)).ToList
                        If (GruposAsig.Count = 0) Then
                            BD.spGrupoSubUsuariosIngresar(CInt(Session("IdUsuario")), CInt(row("sid").ToString), CInt(it.Value.ToString), Session("Usuario").ToString)
                        End If
                    ElseIf (it.Index = valor And it.Selected = False) Then
                        GruposAsig = BD.spGrupoSubUsuariosConsultar2(CInt(Session("IdUsuario")), CInt(row("sid").ToString), CInt(it.Value.ToString)).ToList
                        If (GruposAsig.Count > 0) Then
                            BD.spGrupoSubUsuariosEliminar(CInt(Session("IdUsuario")), CInt(row("sid").ToString), CInt(it.Value.ToString))
                        End If
                    End If
                Next
    ...
    Private Sub LlenarBox()
        Try
            ...
            ListBoxGrupoSub2.Columns.Clear()

            For Each grp_ In GruposDet
                contadorG = 0
                If (tableG.Rows.Count = 1) Then
                    For Each row As DataRow In tableG.Rows
                        GruposAsig = BD.spGrupoSubUsuariosConsultar2(CInt(Session("IdUsuario")), CInt(row("sid").ToString), grp_.IdGrupo).ToList
                    ...
                    If (contadorG >= 1) Then
                        If IsNothing(ListBoxGrupoSub2.Items.FindByValue(grp_.IdGrupo.ToString)) Then
                            ListBoxGrupoSub2.Items.Add(New ListEditItem(grp_.Grupo.ToUpper, grp_.IdGrupo.ToString))
                            Dim lstitem As ListEditItem = ListBoxGrupoSub2.Items.FindByValue(grp_.IdGrupo.ToString)
