...
Public Class AsignarGrupoPto2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (miLista.Length > 0) Then
            ...
            tableG.Columns.AddRange(New DataColumn() {New DataColumn("id", GetType(String)), New DataColumn("pid", GetType(String))})
        ...
        Else
            texto = "No se pudo cargar la información de los puntos. Cierre la ventana y vuelva a intentar."
            ...
            For Each grp_ In GruposDet
                ...
                If (tableG.Rows.Count = 1) Then
                    For Each row As DataRow In tableG.Rows
                        GruposAsig = BD.spPuntoGrupoConsultar(grp_.idGrupoPunto, CInt(row("id").ToString), CInt(Session("IdUsuario"))).ToList
                    ...
                    If (contadorG >= 1) Then
                        If IsNothing(ListBoxGrupoDet2.Items.FindByValue(grp_.idGrupoPunto.ToString)) Then
                            ListBoxGrupoDet2.Items.Add(New ListEditItem(grp_.Grupo.ToUpper, grp_.idGrupoPunto.ToString))
                            Dim lstitem As ListEditItem = ListBoxGrupoDet2.Items.FindByValue(grp_.idGrupoPunto.ToString)
            ...
            Try
                For Each row As DataRow In tableG.Rows
                    If (it.Index = valor And it.Selected = True) Then
                        GruposAsig = BD.spPuntoGrupoConsultar(CInt(it.Value.ToString), CInt(row("id").ToString), CInt(Session("IdUsuario"))).ToList
                        If (GruposAsig.Count = 0) Then
                            BD.spPuntoGrupoIngresar(CInt(it.Value.ToString), CInt(row("id").ToString), CInt(Session("IdUsuario")))
                        End If
                    ElseIf (it.Index = valor And it.Selected = False) Then
                        GruposAsig = BD.spPuntoGrupoConsultar(CInt(it.Value.ToString), row("id").ToString, CInt(Session("IdUsuario"))).ToList
                        If (GruposAsig.Count > 0) Then
                            BD.spPuntoGrupoEliminar(CInt(it.Value.ToString), CInt(row("id").ToString), CInt(Session("IdUsuario")))
                        End If
                    End If
                Next
            Catch ex As Exception
