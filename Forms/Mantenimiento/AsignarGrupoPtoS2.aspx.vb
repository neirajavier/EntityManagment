...
Public Class AsignarGrupoPtoS2
            ...
            Try
                For Each row As DataRow In tableG.Rows
                    If (it.Index = valor And it.Selected = True) Then
                        GruposAsig = BD.spGrupoPuntoSubusuarioConsultar(CInt(it.Value.ToString), CInt(Session("IdUsuario")), CInt(row("sid").ToString)).ToList
                        If (GruposAsig.Count = 0) Then
                            BD.spGrupoPuntoSubUsuarioIngresar(CInt(it.Value.ToString), CInt(Session("IdUsuario")), CInt(row("sid").ToString))
                        End If
                    ElseIf (it.Index = valor And it.Selected = False) Then
                        GruposAsig = BD.spGrupoPuntoSubusuarioConsultar(CInt(it.Value.ToString), CInt(Session("IdUsuario")), CInt(row("sid").ToString)).ToList
                        If (GruposAsig.Count > 0) Then
                            BD.spGrupoPuntoSubUsuarioEliminar(CInt(it.Value.ToString), CInt(Session("IdUsuario")), CInt(row("sid").ToString))
                        End If
                    End If
                Next
            Catch ex As Exception
        ...
        Try
            ...
            For Each grp_ In GruposDet
                ...
                If (tableG.Rows.Count = 1) Then
                    For Each row As DataRow In tableG.Rows
                        GruposAsig = BD.spGrupoPuntoSubusuarioConsultar(grp_.idGrupoPunto, CInt(Session("IdUsuario")), CInt(row("sid").ToString)).ToList
                        If (GruposAsig.Count > 0) Then
                            contadorG += 1
                        End If
                    Next
                    ...
                    If (contadorG >= 1) Then
                        If IsNothing(ListBoxGrupoSub2.Items.FindByValue(grp_.idGrupoPunto.ToString)) Then
                            ListBoxGrupoSub2.Items.Add(New ListEditItem(grp_.Grupo.ToUpper, grp_.idGrupoPunto.ToString))
                            Dim lstitem As ListEditItem = ListBoxGrupoSub2.Items.FindByValue(grp_.idGrupoPunto.ToString)
