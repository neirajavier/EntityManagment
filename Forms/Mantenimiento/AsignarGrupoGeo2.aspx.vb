...
Public Class AsignarGrupoGeo2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (miLista.Length > 0) Then
            ...
            tableG.Columns.AddRange(New DataColumn() {New DataColumn("id", GetType(String)), New DataColumn("gid", GetType(String))})
            ...
            End If
        Else
            texto = "No se pudo cargar la información de las geocercas. Cierre la ventana y vuelva a intentar."
            ...
            For Each grp_ In GruposDet
                ...
                If (tableG.Rows.Count = 1) Then
                    For Each row As DataRow In tableG.Rows
                        GruposAsig = BD.spGeocercaGrupoConsultarV2(grp_.idGrupoGeocerca, CInt(row("gid").ToString), CInt(Session("IdUsuario"))).ToList
                    ...
                    If (contadorG >= 1) Then
                        If IsNothing(ListBoxGrupoDet2.Items.FindByValue(grp_.idGrupoGeocerca.ToString)) Then
                            ListBoxGrupoDet2.Items.Add(New ListEditItem(grp_.Nombre.ToUpper, grp_.idGrupoGeocerca.ToString))
                            Dim lstitem As ListEditItem = ListBoxGrupoDet2.Items.FindByValue(grp_.idGrupoGeocerca.ToString)
            ...
            Try
                For Each row As DataRow In tableG.Rows
                    If (it.Index = valor And it.Selected = True) Then
                        GruposAsig = BD.spGeocercaGrupoConsultarV2(CInt(it.Value.ToString), CInt(row("gid").ToString), CInt(Session("IdUsuario"))).ToList
                        If (GruposAsig.Count = 0) Then
                            BD.spGeocercaGrupoInsertar(CInt(it.Value.ToString), CInt(row("gid").ToString), CInt(Session("IdUsuario")))
                        End If
                    ElseIf (it.Index = valor And it.Selected = False) Then
                        GruposAsig = BD.spGeocercaGrupoConsultarV2(CInt(it.Value.ToString), row("gid").ToString, CInt(Session("IdUsuario"))).ToList
                        If (GruposAsig.Count > 0) Then
                            BD.spGeocercaGrupoEliminar(CInt(it.Value.ToString), CInt(row("gid").ToString), CInt(Session("IdUsuario")))
                        End If
                    End If
                Next
            Catch ex As Exception
